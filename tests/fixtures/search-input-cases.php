<?php
declare(strict_types=1);

$deepPercentPayload = '%3Cscript%3Ealert%281%29%3C%2Fscript%3E';
for ($depth = 0; $depth < 70; ++$depth) {
    $deepPercentPayload = str_replace('%', '%25', $deepPercentPayload);
}

return [
    'blocked' => [
        'lossy SVG event payload' => '<svg onload=alert(1)>nova</svg>',
        'HTML image event payload' => '<img src=x onerror=alert(1)>',
        'HTML generic event payload' => '<div onmouseover=alert(1)>nova</div>',
        'HTML slash event payload' => '<div/onmouseover=alert(1)>nova</div>',
        'HTML slash toggle payload' => '<details/ontoggle=alert(1)>nova</details>',
        'HTML event beyond body budget' => '<a ' . str_repeat( 'x', 513 ) . ' onmouseover=alert(1)>nova',
        'SSTI double braces' => '{{7*7}}',
        'SSTI dollar expression' => '${7*7}',
        'SSTI hash expression' => '#{7*7}',
        'template statement' => '{% print(7*7) %}',
        'SSTI beyond body budget' => '{{' . str_repeat( 'a', 513 ) . '}}',
        'percent encoded HTML event' => '%3Csvg%20onload%3Dalert%281%29%3E',
        'double percent encoded HTML event' => '%253Csvg%2520onload%253Dalert%25281%2529%253E',
        'four layer percent encoded script tag' => '%2525253Cscript%2525253Ealert(1)%2525253C%2525252Fscript%2525253E',
        'four layer entity encoded script tag' => '&amp;amp;amp;lt;script&amp;amp;amp;gt;alert(1)&amp;amp;amp;lt;/script&amp;amp;amp;gt;',
        'canonical closure budget exhaustion' => $deepPercentPayload,
        'percent decoding generates invalid UTF-8' => '%C3%28',
        'path traversal' => '../../etc/passwd',
        'backslash path traversal' => '..\\..\\etc\\passwd',
        'entity encoded percent then HTML' => '&#37;3Csvg&#37;20onload&#37;3Dalert&#37;281&#37;29&#37;3E',
        'double entity encoded script tag' => '&amp;lt;script&amp;gt;alert(1)&amp;lt;/script&amp;gt;',
        'fullwidth percent then encoded script tag' => '&#xFF05;3Cscript&#xFF05;3Ealert(1)&#xFF05;3C/script&#xFF05;3E',
        'mathematical bold JavaScript URI' => '𝐣𝐚𝐯𝐚𝐬𝐜𝐫𝐢𝐩𝐭:alert(1)',
        'mathematical script and Letterlike JavaScript URI' => '𝒿𝒶𝓋𝒶𝓈𝒸𝓇𝒾𝓅𝓉:alert(1)',
        'mathematical dotless i in JavaScript URI' => 'javascr𝚤pt:alert(1)',
        'mathematical dotless j in JavaScript URI' => '𝚥avascript:alert(1)',
        'mathematical script tag' => '<𝓈𝒸𝓇𝒾𝓅𝓉>alert(1)</𝓈𝒸𝓇𝒾𝓅𝓉>',
        'fullwidth brackets with mathematical bold script' => '＜𝐬𝐜𝐫𝐢𝐩𝐭＞alert(1)＜/𝐬𝐜𝐫𝐢𝐩𝐭＞',
        'JavaScript URI' => 'javascript:alert(1)',
        'data HTML URI' => 'data:text/html,<svg onload=alert(1)>',
        'VBScript URI' => 'vbscript:msgbox(1)',
        'gopher URI' => 'gopher://127.0.0.1:11211/_stats',
        'file URI' => 'file:///etc/passwd',
        'proc filesystem' => '/proc/self/environ',
        'SQL true expression' => '1 OR 1=1',
        'quoted SQL true expression' => "' OR '1'='1",
        'SQL comment-separated union' => 'UNION/**/SELECT password',
        'SQL stacked drop statement' => "'; DROP TABLE wp_users; --",
        'SQL stacked drop without terminator' => "'; DROP TABLE wp_users",
        'SQL stacked delete statement' => "'; DELETE FROM wp_users; --",
        'SQL stacked select statement' => '1; SELECT * FROM wp_users; --',
        'SQL stacked select without terminator' => '1; SELECT * FROM wp_users',
        'SQL stacked closing parenthesis drop' => "'); DROP TABLE wp_users",
        'SQL stacked double quote closing parenthesis' => '"); DROP TABLE wp_users',
        'SQL stacked closing parenthesis delete' => '1); DELETE FROM wp_users',
        'SQL stacked insert without terminator' => "abc'); INSERT INTO wp_users VALUES ('x')",
        'SQL stacked drop database' => "'; DROP DATABASE wordpress",
        'SQL stacked select expression' => "'; SELECT @@version",
        'SQL stacked replace into' => "'; REPLACE INTO wp_users VALUES (1)",
        'SQL stacked bare semicolon drop' => '; DROP TABLE wp_users',
        'SQL stacked bare closing parenthesis drop' => '); DROP TABLE wp_users',
        'SQL stacked text closing parenthesis drop' => 'abc); DROP TABLE wp_users',
        'SQL stacked comment-separated boundary' => 'abc)/**/;/**/DROP TABLE wp_users',
        'percent encoded stacked bare boundary' => 'abc%29%3B%20DROP%20TABLE%20wp_users',
        'SQL stacked alphabetic prefix drop' => 'abc; DROP TABLE wp_users',
        'SQL stacked alphabetic prefix delete' => 'coat; DELETE FROM wp_users',
        'SQL stacked alphabetic prefix select' => 'abc; SELECT user_pass FROM wp_users',
        'SQL stacked scalar select' => 'abc; SELECT 1',
        'percent encoded stacked scalar select' => 'abc%3B%20SELECT%201',
        'SQL stacked scalar select alias' => 'abc; SELECT 1 AS x',
        'percent encoded stacked scalar select list' => 'abc%3B%20SELECT%201%2C%202',
        'SQL stacked comment-separated function alias' => 'abc;/**/SELECT/**/VERSION()/**/AS/**/v',
        'SQL stacked insert without into' => 'abc; INSERT wp_users VALUES (1)',
        'SQL stacked comment-separated insert without into' => 'abc;/**/INSERT/**/wp_users/**/VALUES/**/(1)',
        'SQL stacked delete with modifier' => 'abc; DELETE LOW_PRIORITY FROM wp_users',
        'SQL stacked comment-separated delete with modifier' => 'abc;/**/DELETE/**/LOW_PRIORITY/**/FROM/**/wp_users',
        'SQL stacked update with modifier' => 'abc; UPDATE LOW_PRIORITY wp_users SET x=1',
        'SQL stacked comment-separated update with modifier' => 'abc;/**/UPDATE/**/IGNORE/**/wp_users/**/SET/**/x=1',
        'SQL stacked drop temporary table' => 'abc; DROP TEMPORARY TABLE wp_tmp',
        'percent encoded drop temporary table' => 'abc%3B%20DROP%20TEMPORARY%20TABLE%20wp_tmp',
        'percent encoded alphabetic statement boundary' => 'abc%3B%20DROP%20TABLE%20wp_users',
        'percent encoded stacked closing parenthesis' => '%27%29%3B%20DROP%20TABLE%20wp_users',
        'MySQL executable comment union' => '1 UNION /*!50000 SELECT*/ user_pass FROM wp_users',
        'MySQL executable comment with star' => '1 /*!50000 UNION SELECT * FROM wp_users*/',
        'MySQL executable comment beyond body budget' => '1 /*!50000 UNION SELECT ' . str_repeat( 'a', 513 ) . '*/',
        'SQL sleep function' => 'SLEEP(5)',
        'SQL benchmark function' => 'BENCHMARK(1000000,MD5(1))',
        'shell substitution' => '$(nslookup example.test)',
        'shell substitution beyond body budget' => '$(' . str_repeat( 'a', 513 ) . ')',
        'C0 control byte' => "nova\x01probe",
        'Unicode bidi control' => "nova\u{202E}probe",
        'invalid UTF-8' => "nova\xC3\x28probe",
        'marker after character 100' => str_repeat('a', 100) . '{{7*7}}',
    ],
    'allowed' => [
        'CJK product text' => '羊毛外套 3C 電子',
        'HTTPS URL product text' => 'https://example.com 商品手冊',
        'Drop Table board game' => 'Drop Table 桌遊',
        'FreeMarker technical book' => 'FreeMarker 教學書',
        'fetch technical book' => 'JavaScript fetch API 書',
        'C++ vector title' => 'C++ <vector> 入門',
        'Windows path manual' => 'C:\\Users\\Nova\\manual.pdf',
        'quoted music title' => "rock 'n' roll 黑膠",
        'plain language and' => 'organic and fresh',
        'USB-C power specification' => 'USB-C and PD=65W',
        'True Blue music title' => 'Black and True Blue 黑膠',
        'online equals only label' => 'online=only 商品',
        'percentage product text' => '100% cotton tee',
        'version punctuation' => '版本 1.2.3.4',
        'braced size label' => 'Nova Mug {Large}',
        'backtick technical title' => 'JavaScript `fetch` guide',
        'family emoji ZWJ title' => '👨‍👩‍👧‍👦 T-shirt',
        'natural semicolon product description' => 'coat; blue size',
        'natural semicolon use phrase' => 'coat; use cold water',
        'natural semicolon show phrase' => 'gift set; show collection',
        'natural semicolon call phrase' => 'starter set; call me maybe',
        'natural semicolon start phrase' => 'coat; start here',
        'natural semicolon lock phrase' => 'bag; lock style',
    ],
];
