...
- 最低 PHP 需求提升至 8.2；Smart Search 搜尋、分析、REST 與資料表 schema 不變。

## [1.5.4] - 2026-09-13

- 修正窄螢幕搜尋報表的網格最小寬度，排行改為單欄並由各表格容器捲動；自訂日期篩選可換行。
- 僅調整後台 CSS；資料庫 schema 1.0.0、REST URL 與搜尋行為不變。
...
- 每次 `/query` 簽發帶一次性事件 ID 的短效 receipt；同一 receipt 重播仍在 repository 去重，
  ...
  查詢補記；已有 client proof 的送出由目的頁驗證 signed receipt，並以相同事件身分補寫或去重，
  不信任 client 自報的完成 marker。
